...
export function registerEnsurePluginTemplates(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        'Check, install, or update MioKit.Plugin.Templates from NuGet. Call before create_plugin, or when the user asks whether the template pack is installed / needs updating. Fails if templates were installed from a local folder.',
    },
    async () => {
